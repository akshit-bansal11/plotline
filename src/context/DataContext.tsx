...
const parseEntry = (id: string, raw: Record<string, unknown>): EntryDoc => {
  const genresThemes = Array.isArray(raw.genresThemes)
    ? raw.genresThemes.filter((v): v is string => typeof v === "string")
    : [];
  const listIds = Array.isArray(raw.listIds)
    ? raw.listIds.filter((v): v is string => typeof v === "string")
    : [];
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    void token; // Ensure token is a dependency for re-fetching
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const next = snapshot.docs.map((docSnap) =>
          parseEntry(docSnap.id, docSnap.data() as Record<string, unknown>),
        );
