...
export type RelationRecord = { targetId: string; type: string; createdAtMs: number; inferred?: boolean };
...
const toRelationKey = (relation: { targetId: string; type: string }) => `${relation.targetId}::${relation.type}`;
const toRelationStateKey = (relation: RelationRecord) => `${relation.targetId}::${relation.type}::${relation.inferred ? "1" : "0"}`;
...
const normalizeRelations = (relations: RelationInput[]): RelationRecord[] => {
    ...
    for (const relation of relations) {
        ...
        const createdAtMs = toMillis(relation.createdAtMs) ?? toMillis(relation.createdAt) ?? Date.now();
...
const buildReachability = (adjacency: Map<string, Set<string>>, sourceId: string): Map<string, number> => {
...
const relationSetSignature = (relations: RelationRecord[]) => relations
...
const reconcileTransitiveRelations = async (uid: string) => {
    ...
    for (const entryDoc of entriesSnapshot.docs) {
        ...
        const normalized = normalizeRelations(Array.isArray(data.relations) ? data.relations : []).filter(
            (relation) => relation.targetId !== entryDoc.id && entryIdSet.has(relation.targetId)
        );
    ...
    for (const sourceId of entryIds) {
        ...
        for (const family of transitiveFamilies) {
            ...
            for (const [targetId, depth] of reachability.entries()) {
                ...
                const previous = current.find((relation) =>
        ...
        finalRelationsBySource.set(sourceId, normalizeRelations([
            ...explicit,
    ...
    for (const sourceId of allSourceIds) {
        ...
        for (const relation of sourceRelations) {
            ...
            finalRelationsBySource.set(targetId, normalizeRelations([
                ...targetRelations,
                {
                    targetId: sourceId,
                    type: inverseType,
    ...
    for (const entryId of entryIds) {
        ...
        updates.push(updateDoc(doc(db, "users", uid, "entries", entryId), {
            relations: payload,
...
export const updateBidirectionalRelations = async (
    ...
    const normalizedOldRelations = normalizeRelations(oldRelations).filter((relation) => !relation.inferred);
    const normalizedNewRelations = normalizeRelations(newRelations).filter((relation) => !relation.inferred);

    const oldMap = new Map(normalizedOldRelations.map((relation) => [toRelationKey(relation), relation]));
    const newMap = new Map(normalizedNewRelations.map((relation) => [toRelationKey(relation), relation]));
