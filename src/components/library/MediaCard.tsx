...
import { ChevronDown, Link, Star } from "lucide-react";
  ...
  const content = (
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 bg-linear-to-t from-neutral-950/95 via-neutral-950/50 to-transparent opacity-100 md:opacity-0 transition-opacity duration-300 md:group-hover:opacity-100" />

        {showStatusControl && status ? (
          <div className="absolute top-3 right-3 z-10" ref={statusMenuRef}>
            <button
              type="button"
              onClick={(event) => {
          ...
          >
            <div
              className={cn(
                ...
                displayRating !== null
                  ? getRatingAccent(displayRating)
                  : "border-neutral-400/40 text-neutral-200",
              )}
            >
              {displayRating !== null ? (
                displayRatingText
                    ...
                    return (
                      ...
                      >
                        <Star
                          ...
                          className={cn(
                            "transition-colors",
                            isActive ? "text-yellow-300" : "text-neutral-600",
                          )}
                          suppressHydrationWarning
                        />
                      </button>
          ...
          >
            ...
            <AnimatePresence>
              {isRelationsOpen && (
                ...
                >
                  <div className="text-xs font-semibold text-neutral-400 mb-2 px-1">
                    Related Entries
                  </div>
                    ...
                    {relations.map((rel) => {
                      ...
                      return (
                        <div
                          ...
                          className="flex flex-col rounded-lg px-2 py-1.5 hover:bg-white/5 transition-colors group/rel cursor-default"
                        >
                          <span className="text-[10px] text-neutral-500 font-medium uppercase tracking-wider">
                            {rel.type}
                          </span>
                          <span className="text-xs text-neutral-200 truncate group-hover/rel:text-white">
                            {match ? match.title : "Unknown Entry"}
                          </span>
                        </div>
                      );
                    })}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 transition-all duration-300 translate-y-0 opacity-100 md:translate-y-2 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100 media-card-info">
          <h3 className="font-medium text-white line-clamp-2 text-shadow-sm media-card-title">
            {title}
          </h3>
