...
export function MyListsModal({
  ...
  const normalizedInitialViewListId = useMemo(() => initialViewListId?.trim() || null, [initialViewListId]);
  const normalizedInitialEditListId = useMemo(() => initialEditListId?.trim() || null, [initialEditListId]);
  const normalizedInitialDeleteListId = useMemo(() => initialDeleteListId?.trim() || null, [initialDeleteListId]);
  ...
  const createdDateLabel = list?.createdAt ? new Date(list.createdAt).toLocaleDateString() : "Just now";
  ...
  const validateListEdits = () => {
    ...
    if (!name) { setError("List name is required."); return null; }
    if (name.length > 80) { setError("List name cannot exceed 80 characters."); return null; }
    const descriptionValue = editDescription.trim();
    if (descriptionValue.length > 250) { setError("Description cannot exceed 250 characters."); return null; }
    if (editTypes.length === 0) { setError("Select at least one category."); return null; }
  ...
  const confirmRemoveItem = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, "users", uid, "lists", activeListId), { updatedAt: serverTimestamp() });
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        {shouldShowTitleTypeBadges && listSupportedTypes.map((type) => (
                          <span
                            key={`title-type-${type}`}
                    ...
                    <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-[0.2em] text-neutral-500">
            ...
            <div className="flex items-center gap-2 shrink-0">
              {!isEditingMetadata && (
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-neutral-900/40 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-300 transition-colors hover:border-white/20 hover:bg-neutral-900/70 hover:text-white"
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full border border-red-500/20 bg-red-500/5 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-300 transition-colors hover:bg-red-500/10 hover:text-red-300"
            ...
            <div className="flex flex-col items-center justify-center h-[50vh] text-neutral-600 gap-8">
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-xs text-neutral-700 uppercase tracking-widest">
                  {shouldShowViewFilters ? "Select a different filter or add items to this list" : "Add items to this list"}
                </p>
              </div>
            </div>
          ) : (
              ...
              {listItems.map((item) => (
                <div key={item.id} className="group relative">
                  {isEditingMetadata && itemsPendingRemovalIds.has(item.id) && (
                    <div className="pointer-events-none absolute inset-0 z-40 rounded-[2rem]">
                      <div className="absolute inset-0 bg-red-500/15 rounded-[2rem]" />
                      <div className="absolute left-[-18%] top-1/2 h-[5px] w-[140%] -translate-y-1/2 rotate-45 bg-red-500/90 shadow-[0_0_28px_rgba(239,68,68,0.55)]" />
                      <div className="absolute left-[-18%] top-1/2 h-[5px] w-[140%] -translate-y-1/2 -rotate-45 bg-red-500/90 shadow-[0_0_28px_rgba(239,68,68,0.55)]" />
        ...
        <div className="space-y-5 text-sm text-neutral-300">
          ...
          {itemsPendingRemoval.length > 0 && (
            <div className="rounded-lg border border-red-500/20 bg-red-500/10 p-4 text-red-200">
              <div>
                {itemsPendingRemoval.length} item{itemsPendingRemoval.length === 1 ? "" : "s"} will be removed from this list.
              </div>
              <div className="mt-1 text-xs uppercase tracking-wider text-red-200/80">
                Categories: {itemsPendingRemovalTypes.map((type) => listTypeLabels[type]).join(", ")}
              </div>
            </div>
          )}
