...
// ─── Internal — hooks
import { useSearchHandlers, type SearchResult } from "@/hooks/useSearchHandlers";
    ...
    <div ref={containerRef} className={cn("relative w-full max-w-xl", className)}>
      ...
      <div className="relative flex items-center bg-zinc-900/50 border border-zinc-800 rounded-full px-4 py-2 focus-within:bg-zinc-900 focus-within:border-zinc-700 transition-all shadow-lg">
        ...
        <div className="flex items-center gap-2">
          {query && (
            <button onClick={clearQuery} className="p-1 text-zinc-500 hover:text-zinc-300">
              <X className="w-4 h-4" />
            </button>
          )}
              ...
              {results.length > 0 ? (
                <div className="py-2">
                  {results.map((result) => (
                    ...
                    >
                      <div className="flex-shrink-0 w-10 h-14 bg-zinc-900 rounded overflow-hidden border border-zinc-800">
                        {result.image ? (
