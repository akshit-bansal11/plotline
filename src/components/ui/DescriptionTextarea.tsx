...
export function DescriptionTextarea({
    value,
    onValueChange,
    maxLengthLimit = MAX_DESCRIPTION_LENGTH_MANUAL,
    className,
    errorText,
    ...props
            ...
            <div className="flex items-center justify-between text-xs px-1">
                {errorText || isOverLimit ? (
                    <div className="flex items-center gap-1.5 text-red-400 font-medium overflow-hidden">
                        ...
                        <span className="truncate">{errorText || `Description exceeds the ${maxLengthLimit} character limit.`}</span>
                    </div>
                ) : (
                    <div />
                )}
                <div className={cn("font-medium select-none shrink-0", isOverLimit ? "text-red-400" : "text-neutral-500")}>
