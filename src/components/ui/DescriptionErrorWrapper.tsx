    ...
    if (!isInvalid) {
        return (
            <div className={cn("relative h-full w-full", className)}>
                {children}
                {actions && <div className="absolute inset-x-0 bottom-0 pointer-events-none z-20">{actions}</div>}
            </div>
        );
    }

    return (
        <div className={cn("relative overflow-hidden rounded-[inherit] h-full w-full ring-2 ring-red-500 ring-inset", className)}>
            <div className={cn("absolute inset-0 blur-md saturate-50 pointer-events-none select-none", contentClassName)}>
                {children}
            </div>
