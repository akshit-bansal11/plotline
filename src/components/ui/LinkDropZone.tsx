...
import { extractUrlFromDragEvent, parseMediaUrl } from "@/lib/parseMediaUrl";
...
export function LinkDropZone({ onResolved, disabled, onRequireAuth }: LinkDropZoneProps) {
  ...
  const handleResolve = useCallback(async (url: string) => {
    if (disabled) {
      onRequireAuth?.();
      return;
    }
  ...
  useEffect(() => {
    const handleDragEnter = (e: DragEvent) => {
      // Only show overlay if the drag contains text (URLs)
