...
const parseFilterParams = (searchParams: URLSearchParams): SearchFilters => {
  ...
  const status: ApiSearchStatus | null =
    rawStatus === "finished" || rawStatus === "airing" || rawStatus === "tba" || rawStatus === "not_yet_aired"
      ? rawStatus
      : null;
...
const resolveSubtype = (type: MediaType, subtype: string | null, lengthMinutes: number | null) => {
  if (type === "movie") {
    ...
    if (typeof lengthMinutes === "number" && lengthMinutes > 0 && lengthMinutes <= 45) return "short_movie";
  }
  return subtype;
};
...
const buildTmdbUrl = (path: string) => {
  ...
  if (!apiKey && !bearerToken) {
    return { url: null, headers: undefined as Record<string, string> | undefined };
  }
...
const fetchTmdbDetails = async (id: number, type: "movie" | "tv") => {
  ...
  const response = await safeFetchJson(endpoint.url, { headers: endpoint.headers });
...
const searchTMDB = async (queryValue: string): Promise<{ results: SearchResult[]; error: string }> => {
...
const fetchOmdbDetails = async (imdbID: string, apiKey: string): Promise<OmdbDetail | null> => {
  const response = await safeFetchJson(`https://www.omdbapi.com/?apikey=${apiKey}&i=${encodeURIComponent(imdbID)}&plot=short`);
  if (!response.ok) return null;
...
const searchOMDB = async (queryValue: string, type?: "movie" | "series"): Promise<{ results: SearchResult[]; error: string }> => {
...
const searchMALAnime = async (queryValue: string): Promise<{ results: SearchResult[]; error: string }> => {
...
const searchMALManga = async (queryValue: string): Promise<{ results: SearchResult[]; error: string }> => {
  const clientId = process.env.MAL_CLIENT_ID;
  if (!clientId) {
...
const searchIGDBGames = async (queryValue: string): Promise<{ results: SearchResult[]; error: string }> => {
  const clientId = process.env.TWITCH_CLIENT_ID;
  ...
  const results: SearchResult[] = (payload || []).map((item) => {
    ...
    const ratingValue = normalizeIgdbRating(item.aggregated_rating ?? item.total_rating ?? item.rating);
...
const itemMatchesFilters = (item: SearchResult, filters: SearchFilters) => {
  ...
  if (filters.platform) {
    if (!item.platforms || !item.platforms.includes(filters.platform)) return false;
  }
