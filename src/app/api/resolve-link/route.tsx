...
import { parseMediaUrl, type ParsedMediaUrl } from "@/lib/parseMediaUrl";
...
const safeFetchHtml = async (url: string, extraHeaders?: Record<string, string>): Promise<string | null> => {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 10000);
  try {
...
async function resolveImdb(parsed: ParsedMediaUrl): Promise<ResolvedMedia | null> {
  ...
  const isAnimeGenre = genres.some((g) =>
  ...
  if (bearerToken || tmdbApiKey) {
    ...
    if (findResponse.ok) {
      ...
      const posterPath = findData.movie_results?.[0]?.poster_path || findData.tv_results?.[0]?.poster_path;
      const tmdbRating = findData.movie_results?.[0]?.vote_average || findData.tv_results?.[0]?.vote_average;
...
async function resolveTmdb(parsed: ParsedMediaUrl): Promise<ResolvedMedia | null> {
  ...
  // Also fetch OMDB for IMDb rating if we have an IMDb ID
  let imdbRating: number | null = null;
  if (data.imdb_id) {
    const omdbApiKey = process.env.OMDB_API_KEY;
    if (omdbApiKey) {
      const omdbRes = await safeFetchJson(`https://www.omdbapi.com/?apikey=${omdbApiKey}&i=${data.imdb_id}&plot=short`);
      if (omdbRes.ok) {
        const omdbData = omdbRes.data as { imdbRating?: string; Response?: string };
...
async function resolveMal(parsed: ParsedMediaUrl): Promise<ResolvedMedia | null> {
  ...
  return {
    ...
    lengthMinutes: malType === "anime" && typeof data.average_episode_duration === "number"
      ? Math.round(data.average_episode_duration / 60) : null,
...
async function resolveNetflixOrPrime(parsed: ParsedMediaUrl): Promise<ResolvedMedia | null> {
  ...
  console.log(`[resolve-link] ${parsed.source} html length:`, html?.length ?? "null (fetch failed / non-200)");
  ...
  if (titleFromPage) {
    ...
    console.log(`[resolve-link] ${parsed.source} TMDB search result:`, byTitle ? byTitle.title : "null");
...
export async function POST(request: Request) {
  ...
  if (!parsed) {
    return NextResponse.json(
      { data: null, error: "Unrecognized link. Supported: IMDb, TMDB, MyAnimeList, Netflix, Prime Video." },
      { status: 400 }
    );
  }
